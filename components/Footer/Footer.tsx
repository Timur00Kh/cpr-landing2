...
export function Footer(): JSX.Element {
  ...
  const setPol = useCallback((e) => {
    if (e) {
      router.replace('[[...slug]]', '/policy');
    } else {
      router.replace('/');
    }
  }, []);
    ...
    <footer className={classes.footer}>
      <div className={classes.footer_inner}>
        ...
        <div className={classes.footer_text}>
          © 2019 ЦПР, Все права защищены.
        </div>
      </div>
      <Modal isOpen={pol} toggle={() => setPol(!pol)} size="lg">
        <ModalHeader toggle={() => setPol(!pol)} />
        <ModalBody>
          <h5 className={classes.policy_h}>Политика конфиденциальности</h5>
          {policyText.split('\n').map((t, i) => (
            // eslint-disable-next-line react/no-array-index-key
            <p className={classes.policy_p} key={i}>
              {t}
            </p>
          ))}
        </ModalBody>

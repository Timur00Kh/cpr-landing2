...
class Slider extends Component {
      ...
      switch (scrollType) {
        case 'arrows': {
          ...
          newIndex = this.items.getIndex(
            direction === 'prev' ? activeIndex - slides : activeIndex + slides
          );
          break;
        }
        case 'dots': {
    ...
    if (centerMode) {
      ...
      for (i = 0; i < half; i += 1) {
        ...
        if (
          ...
          if (el) {
            const transformString = `${alignment} translateX(${
              settings.shift + (this.dim * (slidesToShow + i) - delta) / 2
            }px)`;
            this.updateItemStyle(el, transformString);
          }
        }
  ...
  widthInit = () => {
    ...
    if (SliderRef) {
      ...
      const sliderWidth = centerMode ? offsetWidth - padding * 2 : offsetWidth;
  ...
  render() {
    ...
    const padding =
      typeof centerPadding === 'string' ? centerPadding : `${centerPadding}px`;
    /*  arrow  */
    ...
    return <div className={classNames(settings.className)}>{component}</div>;
  }
}
